...
def total_variation_loss(img):
    diff1 = img[:, :, :, :-1] - img[:, :, :, 1:]  
    diff2 = img[:, :, :-1, :] - img[:, :, 1:, :]  
    loss = (diff1 ** 2).sum() + (diff2 ** 2).sum()  
    return loss * 1e-3


def visualise_layer_filter(model, layer_nmbr, filter_nmbr, num_optim_steps=26, show=False, lr=0.1, optimizer = Adam, low=120, high=190):

    # Generate a random image
    rand_img = np.uint8(np.random.uniform(low=low,
                                          high=high,
                                          size=(224, 224, 3)))
    ...
    optimizer = optimizer([processed_image], lr=lr, weight_decay=1e-5)
    for i in range(1, num_optim_steps):
        ...
        if(show):
            ...
            print(f"Conv Output Mean: {conv_output.mean().item()}")
            print(f"Conv Output Max: {conv_output.max().item()}")
            print(f"Conv Output Min: {conv_output.min().item()}")
